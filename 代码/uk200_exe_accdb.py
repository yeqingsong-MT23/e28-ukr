...
DB_PATH = Path(r"P:\0_Report\00_AMT中间表\UK200_转化漏斗KPI.accdb")
EXPORT_DIR = Path(r"P:\0_Report\00_AMT中间表\16_导出表格")
EXPORT_PATTERNS = ("UK200_转化漏斗KPI*.xlsx", "UK200_转化漏斗KPI*.xls")
...
def wait_for_recent_export(directory: Path, snapshot: dict[Path, int]) -> Path:
    ...
    raise FileNotFoundError(
        f"等待超过 {EXPORT_WAIT_SECONDS} 秒，仍未检测到最近 {EXPORT_RECENT_SECONDS} 秒内生成的 UK200 导出文件：{directory}"
    )


def remove_parent_uk200_kpi_excels(parent_dir: Path) -> None:
    ...
    if removed_files:
        safe_print("已删除父级目录下旧 UK200 基础 Excel：")
        for file_name in sorted(removed_files):
            safe_print(f"- {file_name}")
    else:
        safe_print("父级目录下未发现需要删除的 UK200 基础 Excel。")


def remove_existing_uk200_excels(base_dir: Path) -> None:
    removed_files: list[str] = []
    for pattern in ("UK200*.xlsx", "UK200*.xls"):
    ...
    if removed_files:
        safe_print("已删除旧UK200结果文件：")
        for file_name in sorted(removed_files):
            safe_print(f"- {file_name}")
    else:
        safe_print("当前目录下未发现需要删除的 UK200 开头 Excel。")
...
def main():
    ...
    remove_parent_uk200_kpi_excels(base_dir)
    remove_existing_uk200_excels(base_dir)
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="uk200_batches_", dir=str(base_dir)))
    ...
    try:
        ...
        safe_print(f"已生成最终 UK200 基础表：{final_output}")
        completed = True
    finally:
        if completed:
